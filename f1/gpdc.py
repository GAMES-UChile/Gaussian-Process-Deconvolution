...
class gpc:
    ...
    def plot_posterior_x(self, time_truth=None, ground_truth=None, format=False):
        ...
        if ground_truth is not None:
            plt.plot(time_truth,ground_truth, color='k', label='True speech')
        plt.plot(self.times_x,self.post_mean_x, color='cornflowerblue', label='GPDC mean')
        error_bars = 2 * np.sqrt(np.diag(self.post_cov_x))
        plt.fill_between(self.times_x, self.post_mean_x - error_bars, self.post_mean_x + error_bars, color='cornflowerblue',alpha=0.2, label='95% error bars')
